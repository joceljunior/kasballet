<template>
  <div class="space-y-6 pb-20 md:pb-6">
    ...
    <AppLoading
      v-if="studentStore.loading && studentStore.students.length === 0"
      card
    ...
    >
      <p class="text-sm text-gray-600">
        Mostrando {{ rangeStart }}–{{ rangeEnd }} de {{ totalCount }} aluna{{ totalCount === 1 ? '' : 's' }}
      </p>
      <div v-if="totalPages > 1" class="flex items-center gap-2">
        <button
          type="button"
          class="btn-secondary text-sm py-1.5 px-3 disabled:opacity-50"
          :disabled="studentStore.loading || studentStore.currentPage === 0"
          @click="goPrevPage"
        >
          Anterior
        </button>
        <span class="text-sm text-gray-700 whitespace-nowrap">
          Página {{ studentStore.currentPage + 1 }} de {{ totalPages }}
        </span>
        <button
          type="button"
          class="btn-secondary text-sm py-1.5 px-3 disabled:opacity-50"
          :disabled="studentStore.loading || studentStore.currentPage >= totalPages - 1"
          @click="goNextPage"
        >
          Próxima
        </button>
...
const totalPages = computed(() => Math.max(1, Math.ceil(totalCount.value / studentStore.pageSize)))
const rangeStart = computed(() => {
  if (totalCount.value === 0) return 0
  return studentStore.currentPage * studentStore.pageSize + 1
})
const rangeEnd = computed(() => {
  if (totalCount.value === 0) return 0
  return Math.min(totalCount.value, (studentStore.currentPage + 1) * studentStore.pageSize)
})
...
async function handleSearch() {
  studentStore.filters = buildFilters()
  studentStore.searchQuery = searchQuery.value.trim()
  studentStore.currentPage = 0
...
async function handleFilter() {
  searchQuery.value = ''
  await studentStore.setFilters(buildFilters())
}

async function goPrevPage() {
  await studentStore.prevPage()
}

async function goNextPage() {
  await studentStore.nextPage()
}
</script>
